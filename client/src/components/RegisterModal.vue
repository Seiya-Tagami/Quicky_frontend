...
// emits
const emit = defineEmits(["close", "addMemo"]);
const closeModal = () => {
  emit("close");
};
...
<template>
  <div class="w-[600px] bg-white border rounded-md p-6 absolute top-10 left-1/2 -translate-x-[50%] z-20">
    <div>
      <div class="flex justify-between items-center">
        <h2 class="font-bold text-2xl">登録</h2>
        <button @click="closeModal"><img src="/xmark-solid.svg" class="w-7 h-7 cursor-pointer" /></button>
      </div>
  ...
  <div class="w-screen h-screen absolute inset-0 z-10 bg-[#07070750]" @click="closeModal" />
</template>
