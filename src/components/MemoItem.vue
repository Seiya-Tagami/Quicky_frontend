...
<template>
  <div
    ...
    :class="[props.memo!.isDone && `${props.isDark ? `!border-green-400` : `!border-green-600`}`, props.isDark ? `bg-gray-700 border-blue-400` : `bg-white border-blue-900`]"
  >
    ...
    <div class="max-w-[480px] w-full flex md:justify-between md:flex-row flex-col ml-[36px]">
      <span class="w-fit font-semibold relative text-[18px] duration-300" :class="[props?.isDark ? `text-white` : `text-cyan-900`, props.memo!.isDone && `!text-gray-300`]">
      ...
      <div
        class="flex gap-2 duration-300"
        :class="[props.isDark ? `text-violet-300` : `text-gray-500`,props.memo!.isDone && `${props.isDark ? `!text-gray-400` : `!text-gray-300`}`]"
      >
